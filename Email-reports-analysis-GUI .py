...
def on_cell_click(cell):
    print(f'Cell clicked: {cell['value']}')
...
def main():
    ...
    def extracting():
        ...
        SN_emails = SN_subject_email_filter(Test_XaasIT_emails, check_error_client)

        # Create separate folder for each client and day
        target_folder = WK_DIR / check_error_client['client'] / DATE
        ...
        MESSAGE_FILE_LOAC.configure(text=f'File location -> {target_folder}')
    ...
    # UI elements
    MESSAGE_OPENING = ct.CTkLabel(FRAME_EXTRACT, text='Select a client', font=('Halvetica', 27))
    ...
    # Button
    BUTTON_EXTRACTING = ct.CTkButton(FRAME_EXTRACT, text='Extract 📎', command=extracting, font=('Helvetica', 22))
    BUTTON_EXTRACTING.place(relx=0.5, rely=0.5, anchor=ct.N)

    MESSAGE_SELECT = ct.CTkLabel(FRAME_EXTRACT, text='', font=('Halvetica', 24))
    MESSAGE_SELECT.place(relx=0.5, rely=0.7, anchor=ct.N)
    MESSAGE_FILE_LOAC = ct.CTkLabel(FRAME_EXTRACT, text='', font=('Halvetica', 16))
    MESSAGE_FILE_LOAC.place(relx=0.5, rely=0.8, anchor=ct.N)
